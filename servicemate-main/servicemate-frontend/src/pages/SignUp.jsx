...
const SignUp = () => {
  ...
  // 2. Handle 10-digit Phone formatting
      ...
      <div className="flex min-h-screen items-center justify-center p-8">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md space-y-6 bg-slate-900/50 p-8 rounded-3xl border border-white/10 backdrop-blur-md shadow-2xl">
          <div className="text-center">
            ...
            <p className="text-slate-400 text-sm">Create your ServiceMate profile</p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">
            
            {/* Role Switcher */}
            <div className="flex gap-2 p-1 bg-white/5 rounded-2xl border border-white/10 mb-4">
              {['customer', 'provider'].map(r => (
                <button 
                  key={r} 
                  type="button" 
                  onClick={() => setForm({...form, role: r})} 
                  className={`flex-1 py-2 rounded-xl capitalize font-bold transition-all ${form.role === r ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'text-slate-500 hover:text-white'}`}
                >
                  {r}
                </button>
              ))}
            ...
            <div className="premium-input-wrapper group relative">
              ...
              {!isVerified && (
                <button 
                  ...
                  className="absolute right-2 top-1/2 -translate-y-1/2 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-xl disabled:opacity-50 transition-colors"
                >
            ...
            {/* Specialty / Service Type (Provider Only) */}
            <AnimatePresence>
              {form.role === 'provider' && (
            ...
            {/* Mobile with +91 and 10-digit limit */}
            <div className="premium-input-wrapper group relative">
